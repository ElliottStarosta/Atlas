...
// ─── Answer panel: CSS grid-row animation — zero reflow, GPU only ─────────────
  ...
  return (
    <div
      style={{
        ...
        opacity: isOpen ? 1 : 0,
        transition:
          "grid-template-rows 0.36s cubic-bezier(0.4,0,0.2,1), opacity 0.28s ease",
        overflow: "hidden",
      }}
    >
      {/* min-height:0 lets the grid row collapse fully */}
          ...
          >
            <div
              style={{
                ...
                animation: isOpen ? "sonarPing 2.5s ease-out infinite" : "none",
              }}
            />
          </div>
          <p
...
export default function FAQ() {
  ...
  useEffect(() => {
    ...
    gsap.set([".faq-eyebrow", ".faq-headline", ".faq-subtext", ".faq-cta-card"], {
      opacity: 0, y: 32, force3D: true,
    });
    itemRefs.current.forEach((el, i) => {
      if (!el) return;
      gsap.set(el, { opacity: 0, x: i % 2 === 0 ? -36 : 36, y: 12, force3D: true });
    });
    ...
    const ctx = gsap.context(() => {
      gsap.to([".faq-eyebrow", ".faq-headline", ".faq-subtext"], {
        ...
        opacity: 1, y: 0, duration: 0.7, stagger: 0.1, ease: "power3.out", force3D: true,
      });
      ...
      itemRefs.current.forEach((el) => {
        ...
        gsap.to(el, {
          ...
          opacity: 1, x: 0, y: 0, duration: 0.6, ease: "back.out(1.4)", force3D: true,
        });
      });

      gsap.to(".faq-cta-card", {
        scrollTrigger: { trigger: ".faq-cta-card", start: "top 90%", once: true },
    ...
    <>
      <style>{`
        .faq-section {
          ...
          background: linear-gradient(
            180deg,
            #04111f 0%, #061526 12%, #071d36 28%,
            #082642 48%, #093050 68%, #0a3860 88%, #0b3f6a 100%
          );
          padding-top: 7rem;
          padding-bottom: 9rem;
        }
        ...
        @keyframes faqRay {
          0%,100% { opacity:0.07; } 50% { opacity:0.20; }
        }
        ...
        @keyframes faqSway  { 0%,100%{transform:rotate(-6deg) translateY(0);} 50%{transform:rotate(6deg) translateY(-6px);} }
        @keyframes faqSwayR { 0%,100%{transform:rotate(6deg) translateY(0);}  50%{transform:rotate(-6deg) translateY(-5px);} }
        ...
        .faq-rivets { position:absolute; inset:0; pointer-events:none; z-index:0; }
        .faq-rivets span {
          position: absolute;
        ...
        .faq-trigger:focus-visible { outline: 2px solid #67e8f9; outline-offset: -2px; }
        ...
        .faq-porthole {
          ...
          box-shadow: 0 0 10px color-mix(in srgb, var(--item-color) 20%, transparent),
                      inset 0 1px 0 rgba(255,255,255,0.12);
          transition: box-shadow 0.3s;
        }
        .faq-trigger:hover .faq-porthole {
          box-shadow: 0 0 18px color-mix(in srgb, var(--item-color) 35%, transparent),
        ...
        /* Snappy rotation — no spring needed here */
        .faq-gauge {
        ...
        /* Sonar ping (reused by AnswerPanel) */
        @keyframes sonarPing {
        ...
        .faq-kelp-divider {
          display: flex; align-items: center; justify-content: center;
          height: 22px; gap: 6px; opacity: 0.4;
        }
        ...
        /* CTA card */
        .faq-cta-card {
          position: relative; overflow: hidden; border-radius: 28px; padding: 2.5rem;
          background: linear-gradient(135deg, rgba(3,70,130,0.55) 0%, rgba(4,18,36,0.92) 100%);
          border: 1px solid rgba(56,189,248,0.35);
          box-shadow: inset 0 1px 0 rgba(255,255,255,0.12), 0 0 50px rgba(14,165,233,0.12);
        }
        .faq-cta-card::before {
          content: ''; position: absolute;
          top: -30%; left: -10%; width: 55%; height: 60%;
          border-radius: 50%;
        ...
        .faq-depth-strip {
          ...
          background: linear-gradient(to bottom, transparent, var(--item-color), transparent);
        ...
        .ct-register-btn {
          ...
          font-family: 'Rubik', sans-serif; font-weight: 700; font-size: 0.78rem;
          letter-spacing: 0.08em; text-transform: uppercase;
          color: #e8f8ff; text-shadow: 0 1px 4px rgba(0,0,0,0.4);
        ...
        .ct-register-btn::after {
          ...
          width: min(34%, 1.4rem); aspect-ratio: 1; border-radius: 50%;
          background: radial-gradient(circle at 35% 35%, rgba(255,255,255,0.65), transparent 58%);
          pointer-events: none; mix-blend-mode: overlay;
          animation: btnSpecPulse 4s ease-in-out infinite; z-index: 1;
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .faq-sonar-ping, .faq-bio-glow { animation: none !important; }
        }
      `}</style>
      ...
      <section id="faq" ref={sectionRef} className="faq-section">
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden" style={{ zIndex: 1 }}>
          {RAYS.map((r, i) => (
            <div key={i} className="faq-ray" style={{
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 overflow-hidden" style={{ zIndex: 2, height: "100%" }}>
          {BG_BUBBLES.map((b) => (
            <div key={b.id} className="faq-bg-bubble absolute rounded-full" style={{
              width: b.size, height: b.size, left: b.left, bottom: "-2%",
              "--dur": b.dur, "--del": b.delay, "--bx": b.bx,
              background: "radial-gradient(circle at 34% 27%, rgba(255,255,255,0.52), transparent 60%)",
        ...
        <div className="pointer-events-none absolute inset-0" style={{ zIndex: 2 }}>
          <div style={{ position: "absolute", top: "22%", left: 0, width: "100%", height: "100px", isolation: "isolate" }}>
            <img src="/images/fish-school.png" alt="" className="faq-fish-ltr"
              style={{ width: 160, height: "auto", opacity: 0.12, filter: "drop-shadow(0 0 8px rgba(56,189,248,.22))" }} />
          </div>
          <div style={{ position: "absolute", top: "72%", left: 0, width: "100%", height: "80px", isolation: "isolate" }}>
            <img src="/images/fish-school.png" alt="" className="faq-fish-rtl"
              style={{ width: 120, height: "auto", opacity: 0.1, filter: "drop-shadow(0 0 6px rgba(56,189,248,.18))" }} />
          </div>
        </div>

        {/* Decorative creatures */}
        <img ref={octopusRef} src="/images/octopus.png" alt=""
          className="faq-octopus pointer-events-none absolute"
          style={{ zIndex: 3, right: "2%", top: "8%", width: "min(110px,16vw)", opacity: 0,
            filter: "drop-shadow(0 0 20px rgba(56,189,248,0.4))" }} />
        <img ref={mantaRef} src="/images/manta.png" alt=""
          className="faq-manta pointer-events-none absolute hidden lg:block"
          style={{ zIndex: 3, left: "1%", bottom: "30%", width: "min(180px,18vw)", opacity: 0,
            filter: "drop-shadow(0 0 16px rgba(56,189,248,0.25))" }} />

        {/* Jellies */}
        <img src="/images/jelly-teal.png" alt=""
          className="pointer-events-none absolute hidden md:block"
          style={{ zIndex: 3, left: "3%", top: "15%", width: "min(70px,10vw)", opacity: 0.55,
        ...
        <img src="/images/jelly-gold.png" alt=""
          ...
          style={{ zIndex: 3, right: "3.5%", bottom: "25%", width: "min(65px,9vw)", opacity: 0.5,
            filter: "drop-shadow(0 0 12px rgba(234,179,8,0.28))",
            animation: "faqJellyFloat 7s ease-in-out 1.5s infinite" }} />

        {/* Seaweed */}
        <img src="/images/seaweed-left.png"  alt="" className="faq-seaweed-l pointer-events-none absolute bottom-0 left-0"
          style={{ zIndex: 4, height: "28vh", width: "auto", maxWidth: 80, opacity: 0.55 }} />
        <img src="/images/seaweed-right.png" alt="" className="faq-seaweed-r pointer-events-none absolute bottom-0 right-0"
          style={{ zIndex: 4, height: "25vh", width: "auto", maxWidth: 75, opacity: 0.5 }} />
        <img src="/images/coral-left.png"  alt="" className="pointer-events-none absolute bottom-0 left-8 hidden sm:block"
          style={{ zIndex: 4, height: "16vh", width: "auto", opacity: 0.6 }} />
        <img src="/images/coral-right.png" alt="" className="pointer-events-none absolute bottom-0 right-8 hidden sm:block"
          style={{ zIndex: 4, height: "16vh", width: "auto", opacity: 0.6 }} />

        {/* Caustic + vignette */}
        <div className="pointer-events-none absolute inset-0" style={{ zIndex: 1, background: `
        ...
        <div className="pointer-events-none absolute inset-0" style={{ zIndex: 5,
          background: "radial-gradient(ellipse at 50% 0%, rgba(4,17,31,0.02) 0%, rgba(4,17,31,0.5) 90%)" }} />

        {/* ── CONTENT ── */}
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6" style={{ zIndex: 10 }}>

          {/* Header */}
          <div ref={headerRef} className="text-center mb-14">
            <div className="faq-eyebrow mb-4 flex items-center justify-center gap-2">
              <span className="h-px w-10" style={{ background: "linear-gradient(to right,transparent,rgba(56,189,248,0.5))" }} />
              ...
              <span className="h-px w-10" style={{ background: "linear-gradient(to left,transparent,rgba(56,189,248,0.5))" }} />
            </div>

            <h2 className="faq-headline font-nature leading-[1.06] mb-5"
              style={{ fontSize: "clamp(2.2rem,5.5vw,3.8rem)", color: "#e8fbff",
                textShadow: "0 0 50px rgba(14,165,233,0.35)" }}>
              Questions from the{" "}
              <span className="text-gradient-ocean" style={{ textShadow: "none" }}>deep.</span>
            </h2>

            <p className="faq-subtext font-body text-center mx-auto max-w-xl text-base leading-relaxed"
              style={{ color: "rgba(186,230,252,0.72)" }}>
              We've surfaced the most common ones below. Still have something bubbling up?{" "}
              <a href="#contact" className="underline underline-offset-2 transition-colors" style={{ color: "#38bdf8" }}>
                Send us a message.
              </a>
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2.5 mb-14">
            {faqs.map((f, i) => (
              <div key={i} ref={(el) => { itemRefs.current[i] = el; }}>
                <div className="faq-deep-item" style={{ "--item-color": f.color }}>
                  <div className="faq-depth-strip" />
                  <div className="faq-rivets" aria-hidden>
                    <span /><span /><span /><span />
                  </div>
                  {openIdx === i && <div className="faq-bio-glow" aria-hidden />}

                  <button className="faq-trigger" onClick={() => toggle(i)} aria-expanded={openIdx === i}>
                    <span className="faq-porthole" style={{ "--item-color": f.color }}>
                      <i className={`fa-solid ${f.icon}`} style={{ color: f.color, fontSize: 12 }} aria-hidden />
                    </span>
                    <span className="faq-question">{f.q}</span>
                    <span className={`faq-gauge ${openIdx === i ? "faq-gauge--open" : ""}`} aria-hidden>
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <circle cx="9" cy="9" r="8" stroke={f.color} strokeOpacity="0.4" strokeWidth="1" />
                        <path d={openIdx === i ? "M5 11 L9 7 L13 11" : "M5 7 L9 11 L13 7"}
                          stroke={f.color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </span>
                  </button>

                  <AnswerPanel isOpen={openIdx === i} color={f.color} answer={f.a} />
                </div>
                ...
                {i < faqs.length - 1 && (
                  <div className="faq-kelp-divider" aria-hidden>
                    {[18,12,22,10,16,8,20].map((h, ki) => (
                      <span key={ki} style={{ height: h, "--kd": `${2.2+ki*0.3}s`, "--kdel": `${ki*0.18}s` }} />
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="faq-cta-card">
            <div className="pointer-events-none absolute inset-0 rounded-[28px]"
              style={{ background: "radial-gradient(ellipse 70% 50% at 10% 50%, rgba(56,189,248,0.1), transparent 60%)" }} />
            <div className="relative z-10 flex flex-col sm:flex-row items-center gap-7">
              <img src="/images/crab.png" alt="ATLAS Crab"
              ...
              <div>
                <p className="font-nature mb-2"
                  style={{ fontSize: "clamp(1.5rem,3vw,2rem)", color: "#e8fbff",
                    textShadow: "0 0 30px rgba(14,165,233,0.3)" }}>
                  Still have questions?
                </p>
                <p className="font-body text-sm mb-5" style={{ color: "rgba(125,211,252,0.65)" }}>
                  Our crab is busy debugging. But the humans on the team are happy to chat.
                </p>
        ...
        <div className="pointer-events-none absolute bottom-0 left-0 w-full" style={{ zIndex: 12 }}>
          <svg viewBox="0 0 1440 100" xmlns="http://www.w3.org/2000/svg"
            ...
            <path d="M0,45 C280,100 560,5 840,52 C1060,85 1260,15 1440,45 L1440,100 L0,100 Z" fill="#040f1a" />
          </svg>
        </div>
      </section>
    </>
